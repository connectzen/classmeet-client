...
export default function MessagesCard() {
    ...
    return (
        <div className="hover-card" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: 24, marginBottom: 32 }}>
            <h3 style={{ margin: '0 0 16px', fontSize: 18, fontWeight: 600 }}>Recent Messages</h3>
            <div style={{ display: 'grid', gap: 12 }}>
                {messages.slice(0, 3).map(m => (
                    <div key={m.id} className="hover-card-subtle" onClick={() => !m.is_read && markAsRead(m.id)} style={{ background: m.is_read ? 'var(--surface-2)' : 'rgba(99,102,241,0.1)', border: `1px solid ${m.is_read ? 'var(--border)' : 'rgba(99,102,241,0.3)'}`, borderRadius: 12, padding: '14px 18px', cursor: m.is_read ? 'default' : 'pointer', transition: 'all 0.2s' }}>
